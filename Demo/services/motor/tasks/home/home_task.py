...
from event.motor_event import TaskHomeFinishedEvent
...
class FindHomeTask(BaseSingleMotorTask):
    ...
    def _start_adjustment(self):
        ...
        while not self.__abort_event.is_set():
            self._controller_service.start(controller_id=self.motor.id, steps=1, freq_hz=self.freq_hz, forward=self.__direction)
    ...
    def handle_pin_status_change(self, event: PinStatusChangeEvent):
        ...
        if not event.status and not self.__top_reached:
            self._controller_service.stop(self.motor.id)
    ...
    def execute(self, **kwargs: Unpack[ExecKwargs]) -> None:
        ...
        if at_home:
            ...
            self._controller_service.start(controller_id=self.motor.id, steps=0, freq_hz=self.freq_hz, forward=self.__direction)
